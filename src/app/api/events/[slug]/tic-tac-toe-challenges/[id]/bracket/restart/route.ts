...
import { restartActivityChampionship } from "@/server/games/activityBracketEngine";
...
export async function POST(
  ...
  try {
    const snapshot = await restartActivityChampionship(io, {
    ...
    return jsonError(
      error instanceof Error ? error.message : "Failed to restart championship",
      "VALIDATION_ERROR",
      400,
    );
  }
