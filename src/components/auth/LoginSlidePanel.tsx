...
function slideSrc(src: LoginSlideSource): string {
...
export function LoginSlidePanel({ slides, className = "" }: LoginSlidePanelProps) {
  ...
  const slideSources = items.map(slideSrc);
  ...
  return (
    <div className={`relative overflow-hidden bg-primary ${className}`}>
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-primary/80 to-transparent p-8">
